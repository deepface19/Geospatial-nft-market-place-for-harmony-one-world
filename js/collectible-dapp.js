...
function TncDapp() {
    ...
    this.render = async function(erc1155, id, index, category, hrc1155, id, index, category){
        ...
        let nft = await window.tncLib.getNft(hrc1155, id);
        ...
        if(index){
            ...
            price = ask.hrc1155Address.length == 1 ? ask.pricePerItem[0] : ask.price;

            let decimals = await tncLib.tokenDecimalsHrc20(ask.tokenAddress);
            price = _this.formatNumberString(price, decimals);
            ...
            swapMode = ask.swapMode;
            isBatch = ask.hrc1155Address.length > 1 ? true : false;
            onsale = ask.amount[0];
            ticker = await tncLib.tokenSymbolHrc20(ask.tokenAddress);
            swap = ask.swapMode == 1 || ask.swapMode == 2 ? 'true' : '';
            options = ask.seller.toLowerCase() == tncLib.account.toLowerCase() ? 'true' : '';
            multiplier = ask.hrc1155Address.length - 1;
            seller = ask.seller;
            royalties = _this.formatNumberString(await tncLibMarket.getRoyalties(ask.hrc1155Address[0], ask.id[0]), 2);
        }
    ...
    this.render2 = async function(erc1155, id, amount, price, token, address, sellerAddress, swapMode, index, hasMore, which, isBatch, multiplier){
        ...
        let meta = await tncLib.getErc1155Meta(erc1155);
        let meta = await tncLib.getHrc1155Meta(erc1155);
        ...
        if(which == 'picker' || which == 'request' || which == 'request2'){
            ...
            $('#nftBuyButtonShortcut' + index).on('click', function(e){
                ...
                let _hrc1155 = $(e.target).data('hrc1155');
                let _id = $(e.target).data('id');
                let _name = $(e.target).data('name');
                let _index = $(e.target).data('index');
                ...
                $('#nftBuyHrc1155Address').val( _hrc1155 );
                $('#nftBuyNftId').val( _id );
                $('#nftBuyTitle').text( _name );
                $('#nftBuyIndex').val( _index );
                $('#nftBuyAmount').val('1');
    ...
    this.performBuy = async function(){
        ...
        for(let i = 0; i < ask.erc1155Address.length 
            < ask.hrc1155Address.length; i++){
    ...
    this.loadPage = async function (page){
        ...
        switch (page){
            ...
            default:
                ...
                if(!await tncLib.isHrc1155Supported(_this.getUrlParam('collection'))){
                    _alert('Unsupported collection type.');
                    return;
                }
